                    ...
                    <div class="col-sm-12 col-md-3">
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <select wire:model='purchaseReport' class="form-control">
                                    <option value="0">Reporte de venta</option>
                                    <option value="1">Reporte de compra</option>
                                </select>
                            </div>
                            ...
                            <div class="col-sm-12 mt-3">
                                ...
                                <input type="date" wire:model="fromDate" class="form-control" {{ $reportType == 0 ? 'disabled' : '' }} placeholder="Click para elegir">
                            </div>

                            <div class="col-sm-12 mt-3">
                                <div class="form-group">
                                    <h6>Fecha de fin:</h6>
                                    <input type="date" wire:model="toDate" class="form-control" {{ $reportType == 0 ? 'disabled' : '' }} placeholder="Click para elegir">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    @if ($purchaseReport == 0)
                                        ...
                                        <a class="btn btn-success btn-block {{ count($data) < 1 ? 'disabled' : '' }}" href="{{ url('reportes/excel/' . $userId . '/' .  $reportType . '/' . $fromDate . '/' . $toDate . '/') }}" target="_blank">Exportar Excel <i class="fas fa-file-excel fa-lg"></i></a>
                                    @else
                                        <a class="btn btn-dark btn-block {{ count($data) < 1 ? 'disabled' : '' }}" href="{{ url('reportes/compras/pdf/' . $userId . '/' .  $reportType . '/' . $fromDate . '/' . $toDate . '/') }}" target="_blank">Generar PDF <i class="fas fa-file-pdf fa-lg"></i></a>
                                        <a class="btn btn-success btn-block {{ count($data) < 1 ? 'disabled' : '' }}" href="{{ url('reportes/compras/excel/' . $userId . '/' .  $reportType . '/' . $fromDate . '/' . $toDate . '/') }}" target="_blank">Exportar Excel <i class="fas fa-file-excel fa-lg"></i></a>
                                    @endif
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-bordered table-striped mt-1 text-center">
                                ...
                                <tbody>       
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <h6>{{ number_format($d->quantity) }}</h6>
                                        </td>
                                        <td>
                                            <h6>${{ number_format($d->total, 2) }}</h6>
                                        </td>
                                        <td>
                                            <h6>{{ \Carbon\Carbon::parse($d->date)->format('d-m-Y') }}</h6>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
